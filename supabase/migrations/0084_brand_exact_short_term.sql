-- ---------------------------------------------------------------------------
-- 0084 · La marca vuelve a la búsqueda corta, pero como dato
-- ---------------------------------------------------------------------------
-- 0077 permitía coincidencia exacta de marca con uno o dos caracteres, y 0082
-- la retiró junto con la rama `OR search_normalize(b.name) = termino`. Se
-- retiró bien: esa rama era el patrón que 0072 vino a eliminar —comparaciones
-- con OR sobre varias tablas, que el planificador no puede combinar—. Lo que
-- estaba mal era perder la función, no la forma.
--
-- Vuelve como `code_type`, no como rama. `variant_search_codes` ya era la
-- estructura de identificadores buscables; la marca entra ahí y toda búsqueda
-- corta sigue resolviéndose con UN recorrido sobre UNA columna indexada.
--
-- LA DIFERENCIA QUE IMPORTA: los identificadores casan por PREFIJO y la marca
-- por IGUALDAD.
--
--   «op»  → encuentra el SKU OP-1234, y NO encuentra la marca OPI.
--   «opi» → encuentra la marca OPI entera.
--   «ml»  → sigue devolviendo cero, porque no es prefijo de ningún
--           identificador ni el nombre completo de ninguna marca.
--
-- Un prefijo de marca con dos letras devolvería demasiadas marcas para ser
-- útil, y eso es justo lo que hacía inservible a «ml». La igualdad conserva el
-- caso que sí sirve —marcas cortas como OPI o MAY— sin reabrir el otro.
--
-- Las dos comparaciones usan el mismo índice `text_pattern_ops`: la igualdad
-- también lo usa, así que no hace falta un índice más.
-- ---------------------------------------------------------------------------

begin;

-- ---------------------------------------------------------------------------
-- 1. La marca entra en la fuente de códigos
-- ---------------------------------------------------------------------------
...
union all
-- La marca. Se guarda igual que los demás, pero se consulta por igualdad.
select variant.id, 'brand_exact',
       public.search_normalize(brand.name)
from public.product_variants variant
join public.products product on product.id = variant.product_id
join public.brands brand on brand.id = product.brand_id
where public.search_normalize(brand.name) is not null;

revoke all on public.variant_code_source from public, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2. La regla de la búsqueda corta, en UN solo sitio
-- ---------------------------------------------------------------------------
-- Cuatro superficies la necesitan —el POS, el catálogo público y las dos del
-- admin—. Si cada una la escribiera, en tres migraciones dirían cosas
-- distintas. Es una función SQL de una sentencia: PostgreSQL la incorpora al
-- plan de quien la llama, así que no cuesta una llamada.
create or replace function public.variant_ids_by_short_code(p_term text)
returns setof uuid
language sql
stable
security invoker
set search_path = ''
as $$
  select codigo.variant_id
  from public.variant_search_codes codigo
  where p_term is not null and p_term <> ''
    and (
      -- Identificadores: por prefijo. Teclear «op» busca lo que EMPIEZA por op.
      (codigo.code_type <> 'brand_exact' and codigo.normalized_code like p_term || '%')
      -- Marca: por igualdad. Un prefijo de dos letras devolvería demasiadas.
      or (codigo.code_type = 'brand_exact' and codigo.normalized_code = p_term)
    );
$$;

comment on function public.variant_ids_by_short_code(text) is
  'Qué variantes coinciden con un término de uno o dos caracteres. Los identificadores casan por prefijo; la marca, por igualdad. La ÚNICA definición de la búsqueda corta: si aparece una segunda, acabarán diciendo cosas distintas.';

revoke execute on function public.variant_ids_by_short_code(text) from public;
grant execute on function public.variant_ids_by_short_code(text) to authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 3. Renombrar una marca ahora también cambia sus códigos
-- ---------------------------------------------------------------------------
-- Antes de 0084 la marca no estaba en `variant_search_codes`, así que el
-- disparador de marcas solo reconstruía documentos. Ahora tiene que reconstruir
-- los dos, o buscar por la marca vieja seguiría encontrando.
...
-- ---------------------------------------------------------------------------
-- 4. Las cuatro superficies pasan a usar la definicion unica
-- ---------------------------------------------------------------------------
...
begin
  ...
  productos_coincidentes as materialized (
    ...
    from public.variant_ids_by_short_code(case when v_corto then v_termino else null end) as ids(variant_id)
    join public.variant_search_projection variante on variante.variant_id = ids.variant_id
  ),
...
CREATE OR REPLACE FUNCTION public.admin_product_search(p_query text DEFAULT NULL::text, p_estado text DEFAULT NULL::text, p_active boolean DEFAULT NULL::boolean, p_brand_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 24, p_offset integer DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.admin_relation_search(p_query text DEFAULT NULL::text, p_template_ids uuid[] DEFAULT NULL::uuid[], p_exclude uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 50)
 RETURNS uuid[]
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
-- ---------------------------------------------------------------------------
-- Relleno: los códigos de marca no existen todavía
-- ---------------------------------------------------------------------------
select public.rebuild_variant_search_codes();
