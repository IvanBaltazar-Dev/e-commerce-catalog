-- ---------------------------------------------------------------------------
-- 0058 · La tarjeta puede anunciar el precio mayorista
-- ---------------------------------------------------------------------------
-- El mayoreo YA se aplica: `evaluate_cart_v2` lo calcula, la línea lleva su
-- asterisco y el motivo se explica agrupado por producto. Lo que faltaba es
-- ANUNCIARLO antes de que se active: el prototipo pone bajo el precio de la
-- tarjeta «S/ 10.50 desde 3», y sin eso nadie sabe que el descuento existe
-- hasta que ya lo activó por casualidad.
--
-- `pos_variant_search` devolvía `wholesaleMinQuantity` (el umbral) pero no el
-- precio, así que la pantalla podía decir «desde 3» sin poder decir de cuánto.
-- Se añade `wholesalePrice` desde `pos_variant_prices`, que es la misma fuente
-- del precio minorista: no se calcula nada nuevo ni se duplica la regla.
--
-- Se toma el MÍNIMO de las variantes, igual que `priceFrom`: dentro de un mismo
-- esmalte los tonos pueden tener precios distintos, y anunciar uno solo como si
-- fuera el de todos mentiría.
-- ---------------------------------------------------------------------------
