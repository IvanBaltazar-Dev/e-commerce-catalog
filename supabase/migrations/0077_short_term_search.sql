-- ---------------------------------------------------------------------------
-- 0077 · Qué significa buscar con una o dos letras
-- ---------------------------------------------------------------------------
-- Medido con 100 000 productos: buscar «ml» costaba 5 096 ms y devolvía medio
-- catálogo, porque cientos de presentaciones dicen «Frasco 8 ml». Dos problemas
-- distintos con la misma raíz.
--
--   · TÉCNICO. El índice trigrama no puede ayudar con uno o dos caracteres: no
--     hay trigramas que buscar. El planificador recorre la tabla.
--   · DE PRODUCTO, y es el que importa. «ml» no es una búsqueda: es el
--     principio de una. Devolver medio catálogo no es dar un resultado, es
--     tapar el que se buscaba.
--
-- LA REGLA. Por debajo de tres caracteres NO se busca por subcadena en nombres
-- ni descripciones. Se busca donde una o dos letras SÍ significan algo:
--
--   · prefijo de SKU
--   · prefijo de código de barras
--   · prefijo de código interno del producto
--   · prefijo de código de tono
--   · coincidencia EXACTA de marca
--
-- A partir de tres caracteres entra la búsqueda trigrama normal, sin cambios.
--
-- Esto es mejor búsqueda, no una limitación. Teclear «R4» y que salga el tono
-- R4 es lo que se espera; teclear «ml» y que salgan 60 000 filas, no.
--
-- LOS ÍNDICES QUE LO SOSTIENEN. Un prefijo (`x%`) sí es indexable con btree,
-- pero solo con la clase de operadores `text_pattern_ops`: el índice por
-- defecto usa la intercalación de la base y no sirve para comparar patrones.
-- Y tienen que estar sobre `search_normalize(columna)`, que es exactamente la
-- expresión que se compara — el error que 0072 vino a corregir.
-- ---------------------------------------------------------------------------

begin;

create index if not exists product_variants_barcode_prefix_idx
  on public.product_variants (public.search_normalize(barcode) text_pattern_ops)
  where barcode is not null;

create index if not exists products_code_prefix_idx
  on public.products (public.search_normalize(code) text_pattern_ops);

create index if not exists color_shades_code_prefix_idx
  on public.color_shades (public.search_normalize(code) text_pattern_ops)
  where code is not null;

create index if not exists brands_name_exact_idx
  on public.brands (public.search_normalize(name));

-- ---------------------------------------------------------------------------
-- Las dos superficies que reciben términos tecleados a mano
-- ---------------------------------------------------------------------------
...
begin
  ...
  candidatas as (
    select
      ...
      --
      -- Si la regla cambia, cambia en los dos sitios. Por eso la prueba 0075 la fija
      -- comparando esta expresión contra la función, variante a variante, sobre
      -- todas las combinaciones de estado, seguimiento y existencia.
      case
    ...
    where v.is_active
      ...
      -- Por debajo de tres caracteres no hay trigramas que buscar, y además
      -- «ml» no es una búsqueda: es el principio de una. Con uno o dos
      -- caracteres se busca solo donde eso significa algo — prefijo de los
      -- códigos y marca exacta. Ver 0077.
      and (
        v_query is null
        or (v_corto and (
             public.search_normalize(v.sku) like v_prefijo
             or public.search_normalize(v.barcode) like v_prefijo
             or public.search_normalize(p.code) like v_prefijo
             or public.search_normalize(cs.code) like v_prefijo
             or public.search_normalize(b.name) = v_termino
           ))
        or (not v_corto and v.search_document like v_pattern)
      )
  ),
  items as (
  ...
  base_products as materialized (
    ...
    where product.is_active
      ...
      and (
        v_pattern is null
        or (v_corto and (
             public.search_normalize(product.code) like v_prefijo
             or public.search_normalize(brand.name) = v_termino
             or exists (
               select 1 from public.product_variants variant
               where variant.product_id = product.id and variant.is_active
                 and (public.search_normalize(variant.sku) like v_prefijo
                   or public.search_normalize(variant.barcode) like v_prefijo)
             )
           ))
        or (not v_corto and (
             product.search_text like v_pattern
             or exists (
               select 1 from public.product_variants variant
               where variant.product_id = product.id and variant.is_active
