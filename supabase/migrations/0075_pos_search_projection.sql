-- ---------------------------------------------------------------------------
-- 0075 · El POS deja de recalcular lo que ya está materializado
-- ---------------------------------------------------------------------------
-- 0072 puso el documento de búsqueda y bajó el coste de ENCONTRAR. Medido con
-- 100 000 productos, encontrar «esmalte» cuesta 53 ms y devuelve 20 278
-- variantes. La búsqueda entera costaba 7 191 ms. El cuello no era el índice de
-- texto: era todo lo que se calculaba DESPUÉS de encontrar.
--
-- Por cada una de esas 20 278 filas el buscador hacía dos cosas caras:
--
--   1. Llamaba a `variant_effective_availability(v.id, p_branch_id)`, que
--      vuelve a leer `product_variants` e `inventory_stock` — cuando el propio
--      SELECT YA tenía unida la fila de `inventory_stock` de esa sede y
--      guardaba su `available_quantity`. Se consultaba dos veces lo mismo.
--   2. Calculaba el precio con `pos_variant_prices(v.id)`, aunque solo se
--      muestran 24. Se calculaba el precio de 20 278 para enseñar 24.
--
-- LO QUE NO SE TOCA: EL ORDEN. Lo disponible sigue primero y lo agotado al
-- final. Bajar de 7 s quitando ese criterio sería arreglar un problema técnico
-- rompiendo la pantalla: quien vende necesita ver antes lo que puede vender.
--
-- Ordenar así exige conocer la disponibilidad de TODAS las coincidencias, y por
-- eso se sigue calculando para todas — pero leyéndola de la posición que ya
-- está unida, no volviendo a consultarla. El precio, que NO interviene en el
-- orden, se calcula después del corte a 24.
--
-- LA FUENTE DE VERDAD SIGUE SIENDO EL INVENTARIO. Aquí no se guarda ninguna
-- copia: se lee `inventory_stock`, cuya clave es (variant_id, branch_id), así
-- que la fila del join ES la posición de esa sede y no hay nada que agregar.
--
-- La regla de disponibilidad queda ahora escrita en dos sitios —la función y
-- esta expresión—, que es exactamente el tipo de duplicación que se acaba
-- desincronizando. Por eso `0075_pos_search_projection.test.sql` las compara
-- sobre todas las combinaciones de estado editorial, seguimiento de inventario
-- y existencia, y falla si dejan de decir lo mismo.
-- ---------------------------------------------------------------------------
...
begin
  ...
  candidatas as (
    select
      ...
      --
      -- Si la regla cambia, cambia en los dos sitios. Por eso la prueba 0075 la fija con
